...
contract ReentrancyTest is Test {
    ...
    function test_vaultSeedsFromSharedToken() public {
        address token = address(new MockToken());
        VulnerableVault vault = new VulnerableVault(VAULT_SEED, token);
        assertEq(address(vault.TOKEN()), token);
        assertEq(IERC20(token).balanceOf(address(vault)), VAULT_SEED);
    }

    function test_exploitDrainsVaultInOneDeploy() public {
        IERC20 token = IERC20(address(new MockToken()));
        VulnerableVault vault = new VulnerableVault(VAULT_SEED, address(token));

        address whitehat = makeAddr("whitehat");
        address recovery = makeAddr("recovery");

        // Deploying Exploit IS the whole attack. moderator=0 skips the (separate)
        // approve step (registry=0 too, unused when moderator=0); this asserts the
        // reentrancy drain + bounty split.
        vm.prank(whitehat);
        new Exploit(address(vault), recovery, address(0), address(0), address(0));
